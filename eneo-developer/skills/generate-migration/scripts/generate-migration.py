...
def read_migration_file(file_path: Path) -> Tuple[str, bool]:
    ...
    try:
        ...
        click.echo(f"{Colors.RED}❌ Error: Migration file not found: {file_path}{Colors.END}", err=True)
        return "", False
    except Exception as e:
        click.echo(f"{Colors.RED}❌ Error reading file: {e}{Colors.END}", err=True)
        return "", False
...
def detect_tenant_id_columns(content: str, table_name: str) -> bool:
    ...
    pattern = rf'op\.create_table\(["\']' + re.escape(table_name) + r'["\'].*?tenant_id.*?\)'
    return bool(re.search(pattern, content, re.DOTALL))


def detect_space_id_columns(content: str, table_name: str) -> bool:
    """Check if table has space_id column"""
    pattern = rf'op\.create_table\(["\']' + re.escape(table_name) + r'["\'].*?space_id.*?\)'
    return bool(re.search(pattern, content, re.DOTALL))
...
def generate_concurrently_pattern(table_name: str, index_name: str, columns: List[str]) -> str:
...
def generate_backfill_template(table_name: str, column_name: str, default_value: str) -> str:
...
@cli.command()
@click.option('--file', 'migration_file', required=True, help='Path to autogenerated migration file')
@click.option('--add-tenant-indexes', is_flag=True, help='Add tenant_id composite indexes')
@click.option('--use-concurrently', is_flag=True, help='Convert to CONCURRENTLY index creation')
...
def enhance(migration_file, add_tenant_indexes, use_concurrently, dry_run):
    ...
    click.echo(f"{Colors.BLUE}{Colors.BOLD}🔍 Analyzing migration file...{Colors.END}\n")
    ...
    if not tables_created:
        click.echo(f"{Colors.YELLOW}⚠️  No tables detected in migration{Colors.END}")
        click.echo(f"   This might be a column addition or other operation")
        sys.exit(0)
    ...
    for table in tables_created:
        ...
        click.echo(f"  tenant_id: {Colors.GREEN if has_tenant_id else Colors.RED}{'YES' if has_tenant_id else 'NO'}{Colors.END}")
        click.echo(f"  space_id: {Colors.GREEN if has_space_id else Colors.YELLOW}{'YES' if has_space_id else 'NO'}{Colors.END}")
        click.echo()

        # Suggest tenant indexes if missing
        if has_tenant_id and add_tenant_indexes:
            needed_index = f'ix_{table}_tenant_space' if has_space_id else f'ix_{table}_tenant'

            if needed_index not in ''.join(existing_indexes):
                click.echo(f"{Colors.YELLOW}💡 Suggestion: Add tenant_id composite index{Colors.END}")

                if use_concurrently:
                    columns = ['tenant_id', 'space_id'] if has_space_id else ['tenant_id']
                    suggestion = generate_concurrently_pattern(table, needed_index, columns)
    ...
    # Display suggestions
    if suggestions:
        click.echo(f"\n{Colors.BOLD}{Colors.BLUE}📝 Enhancement Suggestions:{Colors.END}\n")
        ...
        if not dry_run:
            click.echo(f"{Colors.YELLOW}⚠️  Auto-injection not implemented yet (safety){Colors.END}")
            click.echo(f"   Please copy the code above and paste into your migration file\n")
        else:
            click.echo(f"{Colors.BLUE}[DRY RUN] No files modified{Colors.END}\n")
    else:
        click.echo(f"{Colors.GREEN}✅ No enhancements needed - migration looks good!{Colors.END}\n")
...
def backfill_template(table, column, default):
    ...
    click.echo(f"{Colors.BLUE}{Colors.BOLD}📋 Backfill Template for {table}.{column}{Colors.END}\n")
...
def index_template(table, columns, name):
    ...
    click.echo(f"{Colors.BLUE}{Colors.BOLD}📋 CONCURRENTLY Index Template{Colors.END}\n")
    click.echo(f"{Colors.YELLOW}Add this to your upgrade() function:{Colors.END}\n")
...
def patterns():
    ...
    click.echo(f"{Colors.BOLD}{Colors.BLUE}🎯 Common Eneo Migration Patterns{Colors.END}\n")
...
def doctor():
    ...
    click.echo(f"{Colors.BLUE}{Colors.BOLD}🏥 Alembic Environment Health Check{Colors.END}\n")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if len(heads) == 1:
                click.echo(f"{Colors.GREEN}✓{Colors.END} Single HEAD detected: {heads[0]}")
            elif len(heads) > 1:
                click.echo(f"{Colors.RED}✗{Colors.END} Multiple HEADs detected ({len(heads)})")
                click.echo(f"{Colors.YELLOW}  Run: uv run alembic merge -m 'merge heads'{Colors.END}")
                for head in heads:
                    click.echo(f"     - {head}")
            else:
                click.echo(f"{Colors.YELLOW}⚠{Colors.END} No HEADs found")
        else:
            click.echo(f"{Colors.RED}✗{Colors.END} Error running alembic heads")
            click.echo(result.stderr)

    except FileNotFoundError:
        click.echo(f"{Colors.RED}✗{Colors.END} uv command not found")
    except subprocess.TimeoutExpired:
        click.echo(f"{Colors.RED}✗{Colors.END} Command timeout")
    ...
    if versions_dir.exists():
        ...
        click.echo(f"{Colors.GREEN}✓{Colors.END} {migration_count} migrations found")
    else:
        click.echo(f"{Colors.RED}✗{Colors.END} Versions directory not found: {versions_dir}")
