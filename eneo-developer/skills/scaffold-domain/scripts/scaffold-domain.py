...
@click.option('--tenant-scoped', is_flag=True, default=True, help='Add tenant_id column (default: True)')
@click.option('--space-scoped', is_flag=True, default=False, help='Add space_id column and Space permissions')
@click.option('--permissions', default='read,edit,delete', help='Comma-separated permissions')
@click.option('--relationships', default='', help='Comma-separated foreign keys (field:table)')
...
def scaffold_domain(
    ...
    if not name.islower() or ' ' in name:
        click.echo(f"❌ Error: Domain name must be lowercase with no spaces: {name}", err=True)
        sys.exit(1)
    ...
    if relationships:
        for rel in relationships.split(','):
            if ':' not in rel:
                ...
                click.echo("   Expected format: field:table (e.g., user_id:users)", err=True)
                sys.exit(1)
    ...
    # Domain layer
    files_created.append(render_template(
    ...
    files_created.append(render_template(
        env, 'entity_repo_impl.py.j2', domain_dir / 'infrastructure' / f'{name}_repo_impl.py', context
    ))

    # Application layer
    files_created.append(render_template(
        env, 'entity_service.py.j2', domain_dir / 'application' / f'{name}_service.py', context
    ))

    # API layer
    files_created.append(render_template(
        env, 'api/entity_models.py.j2', domain_dir / 'api' / f'{name}_models.py', context
    ))
    files_created.append(render_template(
        env, 'api/entity_router.py.j2', domain_dir / 'api' / f'{name}_router.py', context
    ))
    files_created.append(render_template(
        env, 'api/entity_assembler.py.j2', domain_dir / 'api' / f'{name}_assembler.py', context
    ))

    # Table layer
    files_created.append(render_template(
        env, 'table/entity_table.py.j2', domain_dir / 'table' / f'{name}_table.py', context
    ))
    ...
    click.echo("   2. Create Alembic migration: uv run alembic revision --autogenerate -m 'add {name} table'")
...
def render_template(env: Environment, template_name: str, output_path: Path, context: dict) -> str:
