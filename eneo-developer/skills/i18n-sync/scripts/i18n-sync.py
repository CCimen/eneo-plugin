...
def load_json(file_path: Path) -> Tuple[Dict[str, str], bool]:
    ...
    try:
        ...
        click.echo(f"{Colors.RED}❌ Error: File not found: {file_path}{Colors.END}", err=True)
        return {}, False
    except json.JSONDecodeError as e:
        click.echo(f"{Colors.RED}❌ JSON syntax error in {file_path}:{Colors.END}", err=True)
...
def save_json(file_path: Path, data: Dict[str, str], dry_run: bool = False) -> bool:
    ...
    if dry_run:
        click.echo(f"{Colors.YELLOW}  [DRY RUN] Would write to: {file_path}{Colors.END}")
        return True
    ...
    try:
        ...
        click.echo(f"{Colors.RED}❌ Error writing {file_path}: {e}{Colors.END}", err=True)
        # Restore backup
        backup_path = file_path.with_suffix('.json.backup')
...
def find_missing_keys(sv_data: Dict[str, str], en_data: Dict[str, str]) -> Tuple[Set[str], Set[str]]:
...
@click.option('--add-keys', is_flag=True, help='Add missing keys with [TODO: translate] placeholder')
...
def main(check, add_keys, fix, auto_fix, sv_file, en_file, dry_run):
    ...
    click.echo(f"{Colors.BLUE}{Colors.BOLD}🔍 Scanning translation files...{Colors.END}\n")
    ...
    if sv_duplicates:
        ...
        for value, keys in sorted(sv_duplicates.items(), key=lambda x: -len(x[1]))[:5]:
            ...
            click.echo(f"   {Colors.BLUE}💡 Consider consolidating to single key{Colors.END}")

        if len(sv_duplicates) > 5:
            remaining_groups = len(sv_duplicates) - 5
            remaining_keys = sum(len(keys) for value, keys in list(sv_duplicates.items())[5:])
            click.echo(f"   ... and {remaining_groups} more groups ({remaining_keys} keys)")
    ...
    if en_duplicates:
        ...
        if len(en_duplicates) > 5:
            ...
            remaining_keys = sum(len(keys) for value, keys in list(en_duplicates.items())[5:])
            click.echo(f"   ... and {remaining_groups} more groups ({remaining_keys} keys)")
    ...
    if fix or auto_fix:
        if sv_violations:
            ...
            click.echo(f"   Manual fix required (auto-rename not implemented to prevent data loss)")
    ...
    if modified:
        ...
        if sv_saved and en_saved:
            if dry_run:
                ...
                click.echo(f"  2. Compile Paraglide: bun run i18n:compile")
                click.echo(f"  3. Validate: python scripts/i18n-validate.py --full")
        else:
